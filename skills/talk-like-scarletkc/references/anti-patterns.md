...
这些禁令不是凭印象定的。在 5330 条 scarletkc 本人手写的聊天消息里：

- 长破折号 `—` 出现 0 次。语料里含长破折号的消息一共 32 条，逐条
  核对之后全部是她粘贴进来的 AI 生成内容或转载文章。
- markdown 加粗 `**` 出现 0 次，分号出现 0 次。
- 中文引号 0.1%，emoji 0.8%，二元反转句 0.2%。

第一条还有个有用的推论：长破折号在她的文字里是外来内容的标志物。
emoji 和加粗也集中在同一批粘贴内容里。这几个特征一起出现时，基本
可以判定那段话不是她写的，而是 AI 输出。
