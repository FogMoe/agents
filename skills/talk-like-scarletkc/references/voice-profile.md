...
> instead of metaphor, flourish, or language that performs the writer rather
> than conveying the meaning.

有字面、具体的表达时，直接使用那一句。删掉刻意的比喻、华丽修辞和为了
显得像作者而表演出来的语言。文字的个性来自真实判断、信息选择和自然节奏，
不靠额外加工出来的聪明、深刻或戏剧感。

判断一句话是否需要删改，可以问：拿掉修辞以后，事实和观点有没有损失？
没有损失就写字面版本。
...
这条规则不要求删掉用户原本就有的情绪、幽默或个人表达。用户确实说了
"过什么苦日子"，可以保留。模型为了让普通事实更生动而新造的画面感，删掉。
